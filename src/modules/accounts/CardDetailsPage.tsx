...
export default function CardDetailsPage() {
  ...
  const { cards, activities: allActivities } = useFinance();
  const id = routeParam;
  const card = cards.find(c => c.id === id);

  const cardTransactions = useMemo(() => 
    allActivities.filter(a => a.sourceAccountId === id),
  [allActivities, id]);
  ...
  }, [cardTransactions, sortField, sortDirection, searchQuery, filterType, filterStatus, dateRange]);
  ...
  return (
    <PageLoadingState isLoading={loading}>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-6 md:mb-8 gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold mb-1 md:mb-2">Card ending in {card.number.slice(-4)}</h1>
          <p className="text-sm md:text-base text-gray-500">Card Details & Recent Transactions</p>
        </div>
        <div className="text-left md:text-right">
          <p className="text-gray-500 text-xs md:text-sm mb-1">Outstanding Balance</p>
          <h2 className={cn("text-3xl md:text-4xl font-bold", (card.balance ?? 0) > 0 ? "text-green-600" : "text-red-600")}>
